defmodule KafkaEx.GenConsumer do
  ...
  **NOTE** `:commit_interval`, `auto_commit_reset` and `:commit_threshold` default to the
  ...
  def terminate(_reason, %State{} = state) do
    Logger.debug(fn -> "### #{inspect(state.worker_name)} commiting state" end)
    commit(state)
    Logger.debug(fn -> "### #{inspect(state.worker_name)} commited state, unlinking" end)
    Process.unlink(state.worker_name)
    Logger.debug(fn -> "### #{inspect(state.worker_name)} unlinked, stopping worker" end)
    KafkaEx.stop_worker(state.worker_name)
    Logger.debug(fn -> "### #{inspect(state.worker_name)} done stopping" end)
  end
